...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if(formJoueur){
    formJoueur.addEventListener('submit', async (e) => {
      ...
      if(!photoFile){ alert("Ajoute une photo"); return; }
...
window.logout = function() { showPage('accueil'); }  const snapshot = await photoRef.put(photoFile);
  const photoURL = await snapshot.ref.getDownloadURL();
  const prix = form.code_promo.value.toUpperCase() === 'KOFFI25'? 75 : 100;
